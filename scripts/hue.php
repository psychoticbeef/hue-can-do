...
//			return (string)$k['state']['ct'] != (string)$params['ct'];
//		});

		foreach ($bulbs as $bulb => $value) {
			$min_ct = 153;
			$max_ct = ($value['type'] === 'Color temperature light') ? 370 : 500;
			$params['ct'] = min($params['ct'], $max_ct);
			$params['ct'] = max($params['ct'], $min_ct);
			if ((string)$params['ct'] == (string)$value['state']['ct']) continue;
			echo 'curval: ' . $value['state']['ct'] . ' --- param: ' . $params['ct'] . PHP_EOL;
